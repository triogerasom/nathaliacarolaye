...
const { fetchProcesses } = require('../api/fiemg-import.js');
...
function executeSql(sql, env) {
  return new Promise((resolve, reject) => {
    ...
    child.stdin.end(`SET standard_conforming_strings = on; SET statement_timeout = '90s';\n${sql}`);
  });
}

function syncSql(processes, runUrl) {
...
INSERT INTO public.fiemg_opportunities (company_cnpj, process, object, entity, external_process_id, process_number, modality, portal_status, portal_group, item_count, deadline, estimated_value, status, source_url, imported_at)
SELECT '${COMPANY_CNPJ}', doc->>'processNumber', doc->>'object', doc->>'entity', (doc->>'externalProcessId')::bigint, doc->>'processNumber', doc->>'modality', doc->>'portalStatus', doc->>'portalGroup', (doc->>'itemCount')::integer, (doc->>'deadline')::timestamptz::date, (doc->>'estimatedValue')::numeric, 'Mapeando', doc->>'sourceUrl', now()
FROM incoming_fiemg
...
UPDATE public.fiemg_sync_status SET status = 'success', finished_at = now(), last_success_at = now(), process_count = ${processes.length}, item_count = ${itemCount}, message = 'Sincronização automática concluída', run_url = ${sqlLiteral(runUrl)} WHERE company_cnpj = '${COMPANY_CNPJ}';
COMMIT;
...
async function main() {
  ...
  try {
    const processes = await fetchProcesses({ limit: 100, includeItems: true });
    ...
    const verification = await executeSql(syncSql(processes, runUrl), env);
